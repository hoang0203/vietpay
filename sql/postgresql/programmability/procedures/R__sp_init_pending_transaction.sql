...
BEGIN
    FOR rec IN 
        ...
        WHERE pg_namespace.nspname = 'operation'  -- Change to your schema
          AND proname = 'sp_init_pending_transaction'   -- Change to your procedure name
    LOOP
...
CREATE OR REPLACE PROCEDURE operation.sp_init_pending_transaction(
    p_tx_id UUID,
    p_idemp_key VARCHAR,
    p_amount NUMERIC,
    p_source_id UUID,
...
BEGIN
    ...
    WHERE id IN (p_source_id, p_suspense_id) 
    ...
    VALUES (p_idemp_key, 'PROCESSING', NOW() + INTERVAL '1 hour');

    INSERT INTO operation.transactions (id, idempotency_key, description, status)
    VALUES (p_tx_id, p_idemp_key, p_desc, 'PENDING');
    ...
    VALUES (gen_random_uuid(), p_tx_id, p_source_id, -p_amount, 'VND'),
           (gen_random_uuid(), p_tx_id, p_suspense_id, p_amount, 'VND');

EXCEPTION 
    WHEN OTHERS THEN
        RAISE EXCEPTION 'sp_init_pending_transaction thất bại: %', SQLERRM;
END;
$$;